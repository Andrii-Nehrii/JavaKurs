...
public class StreamExample2 {
    ...
    Optional<T> max(Comporator <T> comporator) - самое большон значение по компоратору
    Optional<T> min(Comporator <T> comporator) - самое маленькое значение по компоратору
...
//        task11();
    ...
    private static void task16() {
       ...
       List<Integer> integers = Arrays.stream(ints)
        ...
        int[] intToArray = integers.stream()
                .mapToInt(i -> i)
                .toArray();
    ...
    private static void task13() {
        List<Integer> integers = List.of(2, 435, 7, 8, 8, 9);
    ...
    private static void task12() {
        ...
        Optional<String> catStream = cats.stream()
                .map(Cat::getName)
                .max(Comparator.comparing(String::length));
        System.out.println(catStream.orElse(null));
